...
- 2026-09-05：继续按 `docs/spikingformer_PROMPT.md` 实现 ImageNet pretrained Spikingformer 的 SOMA 回放。已在项目内只读副本载入官方仓库/checkpoint 与一张 ImageNet 预处理样例，CPU golden 仅将官方硬编码 `cupy` backend 替换为等价 `torch`。`tools/prepare_spikingformer_imagenet_assets.py` 已导出 `[4,1,96,224,224]` 的 `proj1_lif` binary 边界、官方预测 258、Conv/Linear 参数、BN fold 与逐层 LIF/BN trace；修正了无 bias Conv 的 BN-fold bias（必须包含 `-running_mean`）。C++ 已加入默认 `input_scale=1`（Spikingformer 可配 .5）和通用 `timestep_spike_attention`：Q/K/V 三路经既有 operand/NoC 对齐，在目标 Core 暂态计算 `Q @ (K^T @ V) * scale`，KV 不跨 timestep/NoC；summary/CSV 新增 KV/Q attention update 分列，focused test 已通过。按 Loihi2-like Cx State 语义，runtime 命名为 `neuron_model: multi_valued_state` / `local_state_buffer`：每个 neuron 的本地可编程 multi-valued state 只在当前 logical timestep 有效，直接写入下游当前 buffer，不产生 spike/packet、不会访问 NoC 或跨 timestep保持；Core-level shortcut→pre-LIF 测试通过。全图 runtime/mapping 已生成（132 layer、163 connection、35,280 physical Core partitions），下一步实际运行与逐元素 comparator；当前尚未声称 Spikingformer end-to-end 已完成。
  - 实际 full run 已进入主循环；启动后 RSS 由 7.7 GiB 增至 8.6 GiB 后进程消失，未生成 `summary.json`。后续内核日志显示 WSL distribution 被终止并重新初始化，未见 OOM-killer 记录，故仅记录为“宿主/WSL 中断”，不把它归因于 simulator OOM。已确认没有展开 neuron-to-neuron connectivity：Conv 使用 Spatial Pattern Template。修正两项：`multi_valued_state` Core 不再分配未使用的 LIF/ST-BIF persistent SomaState/last-state SoA；四个 binary MaxPool 改为 `channelwise` Spatial Pattern（kernel 3、stride 2、padding 1、weight 1），避免错误跨通道广播。重新生成 runtime/mapping 后构建与 focused test 通过；待在可持续命令会话中再次运行完整 sample。
