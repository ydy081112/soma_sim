...
当前事件语义在每个到达 spike 的 Core update 内直接完成 threshold 与 firing；多个 firing 按 neuron id 升序分别生成普通 Data spike。每个逻辑 timestep（包括没有外部 spike 的 timestep）都会为带 bias 的层加入一个 `Bias` 事件。timestep 之间有 barrier，但单个 timestep 内仍采用逐 spike 的事件驱动 threshold；它与按 timestep 聚合完整卷积后只 threshold 一次的转换参考并非逐值等价。`reference/` 中保留了 SANA-FE/Python 的已知结果用于后续逐层校准。
