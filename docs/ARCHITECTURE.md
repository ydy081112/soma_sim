...
当 `core.input_fifo` 启用时，destination router 下的输入释放队列按 physical Core 分离；每个 Core 的 FIFO 数量和深度来自 `fifo_num_per_core` / `fifo_depth_per_core`。资源表只保存各 FIFO 中 packet 的释放时刻与 processing service，并用 destination flow 的在途 service 计算回压，不创建 FIFO event 或第二套 scheduler；Local ejection free time 同样按 Core 独立。当前实现只支持 `fifo_per_core: true`，关闭 `input_fifo` 时沿用普通 output/link free-time 路径。
...
同步模式中，timestep `t` 的外部 input 先以 timestep start 作为 generation time 入队，同时所有 Core 从该时刻开始 neuron processing：按 neuron id 升序读取 `t-1` 留下的 buffer，访问全部 mapped neurons，只对 pending input、非零 bias 或已有 membrane state 的 neuron 做实际 state update。每个 neuron 每步最多 firing 一次，soft reset 后剩余电位留给后续 timestep；产生的 spike 标记为 `t`，并以自身 `soma_fire` 完成时刻作为 generation time。所有 Data 随后按真实 generation time 在 global queue 中交错经过 axon-out/NoC/synapse，只写入 `timestep_buffer` 供 `t+1` 使用；排空本 timestep queue 后才执行 barrier。共享 Core/NoC 的 free-time 资源仍决定两条时间线相遇后的等待，不使用全局 phase-start clamp。
...
Mapping 中的 layer 使用起始 `pe/core` 加连续分区表达物理放置；`aggregate_core_count` 必须与 `ceil(neurons/max_neurons)` 一致。空间层可以保留 spatial-major 逻辑索引，同时用 `physical_neuron_order: channel_major` 对齐参考 mapping。Synapse latency 按 packet 在 destination Core 内实际发生的 local updates 串行累计，并按 destination op 选用 spatial 3.1 ns 或 dense 3.8 ns；不会把全层 updates 串成一个资源，也不会展开完整连接图。
